...
import { useProductVariantsMap } from '@/hooks/use-catalog';
 ...
 * the rest are added directly. Products with active variants (flavors)
 * ALWAYS open the drawer — the flavor is mandatory before add-to-cart,
 * even for beverages that would otherwise add directly.
 *
...
export function useAddProduct(product: Product, productForCustomization: Product = product) {
  ...
  const hasVariants = (variantsMap?.[product.id]?.length ?? 0) > 0;
  ...
  const handleAddClick = () => {
    if (hasVariants || isProductCustomizable(productForCustomization)) {
      setDrawerOpen(true);
      return;
    }
